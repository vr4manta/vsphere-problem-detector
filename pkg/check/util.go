...
func GetVCenter(checkContext *CheckContext, node *v1.Node) (*VCenter, error) {
	if checkContext.PlatformSpec != nil {
		// Determine which vCenter connection is in by looking at failure domain info
		// failure-domain.beta.kubernetes.io/region: us-east
		// failure-domain.beta.kubernetes.io/zone: us-east-4a
		...
		if len(region) > 0 && len(zone) > 0 {
			...
			// Node is not configured w/ FD labels.
